...
export function PrizeStatusFilters({
  prizes,
  promoters,
  events
}: {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    router.push(`/estado-premio${params.toString() ? `?${params.toString()}` : ""}`);
  }
    ...
    <form onSubmit={submit} className="mb-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <select name="estado" defaultValue={searchParams.get("estado") || ""} className="rounded-md border border-slate-300 px-3 py-2">
          <option value="">Pendientes y enviados</option>
        ...
        <input
          ...
          placeholder="Código, participante, teléfono, NIE, correo"
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <button
          type="button"
          onClick={() => router.push("/estado-premio")}
